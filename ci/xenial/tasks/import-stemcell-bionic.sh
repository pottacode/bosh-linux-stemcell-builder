...
for (( i=1; i<=60; i++ ))
do
  pr_image_id=$(slcli image list --name "${private_image_name}" | tail -f | cut -d " " -f 2)
  echo ${pr_image_id} > id.txt
  private_image_id="$(cat id.txt)"
...
for (( i=1; i<=60; i++ ))
do
  size=$(slcli image detail ${public_image_id} | grep disk_space | awk '{print $2}')
  if [[ ${size} == "0" ]]; then
